...
RUN ./apps/dashboard/node_modules/.bin/prisma generate --schema=apps/dashboard/prisma/schema.prisma
...
RUN mkdir -p apps/dashboard/dist_server && \
    ...
    --format=cjs \
    --packages=external \
    --outfile=apps/dashboard/dist_server/server.js
...
CMD ["sh", "-c", "./apps/dashboard/node_modules/.bin/prisma db push --schema=apps/dashboard/prisma/schema.prisma --accept-data-loss && node apps/dashboard/dist_server/server.js"]
