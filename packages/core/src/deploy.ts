...
async function detectProjectRuntime(client: SSHClient, releasePath: string): Promise<RuntimeDetectionResult | null> {
    const candidatePaths = [releasePath];
    const { stdout: directories, code } = await client.exec(`find ${releasePath} -mindepth 1 -maxdepth 2 -type d ! -name node_modules ! -path '*/node_modules/*'`);
    if (code === 0 && directories.trim()) {
        candidatePaths.push(...directories.split('\n').map(dir => dir.trim()).filter(Boolean));
    }
